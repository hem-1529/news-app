  ...
  useEffect(() => {
    async function fetchCommands() {
      const [commandsRes, categoriesRes] = await Promise.all([
        supabase.from('windows_commands').select('*').eq('type', 'cmd').order('command', { ascending: true }),
        supabase.from('categories').select('name').eq('page_type', 'cmd').order('sort_order', { ascending: true }),
      ])
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-4 py-10">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">コマンドプロンプト一覧</h1>
            ...
            >
              ...
              {activeItem.example && (
                <div>
                  ...
                  <pre className="bg-gray-900 text-gray-300 text-sm rounded-lg px-4 py-3 overflow-x-auto font-mono whitespace-pre-wrap">
                    {activeItem.example}
                  </pre>
                </div>
              )}
